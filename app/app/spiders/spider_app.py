...
from app.items import LookItem, LocationItem
...
class AppSpider(Spider):
	...
	def parse_look(self, look):
		...
		item['orig_hype'] 	= look.xpath('div[@class="look_hype_container"]/div[@data-look-id="'+ id +'"]/@data-hypes-count').extract()[0]
		return item

	def parse_location(self, look):
		item 	    		= LocationItem()
		try:
			item['city'] 	= look.xpath('div[@class="look_meta_container"]/p/a[starts-with(@data-page-track,"location")]/text()').extract()[0]
		except IndexError:
			logger.warn("No city for this one.")
		item['country'] 	= look.xpath('div[@class="look_meta_container"]/p/a[starts-with(@data-page-track,"country")]/text()').extract()[0]
		return item
